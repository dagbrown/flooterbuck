...
#
# $Id: babel.pm,v 1.6 2006/10/08 04:05:10 rich_lafferty Exp $
#------------------------------------------------------------------------
...
sub scan(&$$)
{
	...
	if (::getparam('babel') && ($message =~ /^translate/)) {
		if ($no_babel) {
			$callback->('translate requires URI::Escape and LWP::UserAgent');
			return 'NOREPLY';
		}
    ...
    # default language is English
		...
		if (!defined($through_lang)) {
			if ($lang_pairs{$from_lang} !~ /$to_lang/) {
				$callback->("babelfish cannot translate from $from_lang to $to_lang.  Try translating through English.");
				return 'NOREPLY';
			}
		} else {
			if ($lang_pairs{$from_lang} !~ /$through_lang/) {
				$callback->("babelfish cannot translate from $from_lang to $through_lang.  Try translating through English.");
				return 'NOREPLY';
			} elsif ($lang_pairs{$through_lang} !~ /$to_lang/) {
				$callback->("babelfish cannot translate from $through_lang to $to_lang.  Try translating through English.");
				return 'NOREPLY';
			}
		}

		# we can only handle Japanese if we have Jcode.
		if ((($from_lang eq "ja") || ($to_lang eq "ja")) && ($no_japanese)) {
			$callback->("This particular infobot is not configured for Japanese");
			return 'NOREPLY';
		}
		...
		if (!defined($through_lang)) {
			...
			my $intermediary = &translate($message, $from_lang, $through_lang);
			$callback->(&translate($intermediary, $through_lang, $to_lang));
		}
