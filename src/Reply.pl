...
sub getReply {
    ...
    if ( $result = get( "is", $locMsg ) ) {
        ...
        if ( $param{'VERBOSITY'} > 1 ) {
            ...
            $debugstring .=
              "\tX[$X] verb[$theVerb] det[$y_determiner] Y[$Y]\n";
    ...
    if ( $V ne "" ) {                 # if there was a head...
        if ( not $literal )
        {    # Changed to cope with $msgFilter - 26Jun19100, Masque
            my (@poss) = split( /(?<!\\)\|/, $result );
            $poss[0] =~ s/^\s//;
            $poss[$#poss] =~ s/\s$//;
            my @filtered = grep /\Q$msgFilter\E/, @poss
              unless $msgFilter eq "NOFILTER";
            ...
            if (@filtered) {
                ...
                if (   $msgFilter eq "NOFILTER"
                    || $result =~ /\Q$msgFilter\E/ )
                {
                    $theMsg = $result;
                } else {
                    $theMsg =
                      q!<reply>Hmm.  No matches for that, $who.!;
                }
            }
            $theMsg =~ s/\\\|/\|/g;
        } else {
    ...
    if ( $theMsg ne "" ) {
        if ( $msgType =~ /public/ ) {
            ...
            {
                &status("repeat ignored ($interval secs < "
                      . getparam('repeatIgnoreInterval')
                      . ")" );
                $skipReply = 1;
                $theMsg    = "NOREPLY";
    ...
    # specially defined type.  get and process an RSS (RDF Site Summary)
            ...
            if ( $rdf_loc !~ /^(ht|f)tp:/ ) {
                &msg( $who,
"$orig_Y: bad RSS [$rdf_loc] (not an HTTP or FTP location)"
                );
            } else {
        ...
        } elsif ( $msgType !~ /private/
            and $theMsg =~ s/^\s*<action>\s*(.*)/\cAACTION $1\cA/i )
        {
 ...
 # specially defined type.  only remove '<action>' and make it an action
            ...
            if ( !$infobots{$nuh} and $theVerb =~ /is/ ) {
                ...
                if ( $x == 8 ) {
                    $theMsg =
                      "it has been said that $orig_Y is $theMsg";
                }
                if ( $x == 9 ) {
