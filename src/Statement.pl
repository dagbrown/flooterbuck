...
sub doStatement {
    ...
    $in =~
      s/^no,\s+//i;    # don't want to complain if it's new but negative
...
# Whoops!  Better make sure that we're adding karma to something existant.
    ...
    if ( $in =~
        /\b(news|http|ftp|gopher|telnet):\s*\/\/[\-\w]+(\.[\-\w]+)+/ )
    {
        $theType = $1;
    }

    # here's where you set the behaviour.
    if (   ( getparam('acceptUrl') =~ /\d+/ )
        && $addressed
                ...
                if ( $msgType =~ /public/ ) {
                    if ($addressed) {
                        if ( rand() > 0.5 ) {
                            ...
                            &performSay(
"i'm sorry, but that entry is too long, $who"
                            );
                        }
                    }
                } else {
       ...
       # the arsenal of things to ignore if we aren't addressed directly
                ...
                $lhs =~ /^(who|what|when|where|why|how|it) /i
                  and $failed++;
                $lhs =~ /^(this|that|these|those|they|you) /i
                  and $failed++;
                ...
                $lhs =~ /^[\[<\(]\w+[\]>\)]/
                  and $failed++;    # [nick] from bots
                $lhs =~ /^heya?,? / and $failed++;    # greetings
                $lhs =~ /^\s*th(is|at|ere|ese|ose|ey)/i
                  and $failed++;                      # contextless
                $lhs =~ /^\s*it\'?s?\W/i
                  and $failed++;    # contextless clitic
                $lhs =~ /^\s*if /i and $failed++;    # hypothetical
                $lhs =~ /^\s*how\W/i
                  and $failed++;    # too much trouble for now
                $lhs =~ /^\s*why\W/i
                  and $failed++;    # too much trouble for now
                $lhs =~ /^\s*h(is|er) /i and $failed++;    # her name is
                $lhs =~ /^\s*\D[\d\w]*\.{2,}/ and $failed++;    # x...
                $lhs =~ /^\s*so is/i
                  and $failed++;    # so is (no referent)
                $lhs =~ /^\s*s+o+r+[ye]+\b/i and $failed++;    # sorry
                $lhs =~ /^\s*supposedly/i    and $failed++;
                $lhs =~ /^all /
                  and $failed++;   # all you have to do, all you guys...
