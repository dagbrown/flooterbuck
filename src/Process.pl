...
sub process {
    ...
    if ( $msgType =~ /private/ and $message =~ /^hey, what is/ ) {
        $infobots{$nuh} = $who;
        &msg( $who,
"inter-infobot communication now requires version 0.43 or higher."
        );
        return 'INTERBOT';
    }
    ...
    return 'INTERBOT'
      if $message =~ /^told /;    # reply from friendly infobot
    return 'INTERBOT'
      if $message =~ /^told /;    # reply from friendly infobot
    ...
    if ( $infobots{$nuh} ) {
        if ( $msgType =~ /private/ ) {
            if ( $message =~ /^QUERY (<.*?>) (.*)/ ) {
                ...
                if ( $r = &get( "is", $item ) ) {
                    &msg( $who,
                        ":INFOBOT:REPLY $target $item =is=> $r" );
                }
                if ( $r = &get( "are", $item ) ) {
                    &msg( $who,
                        ":INFOBOT:REPLY $target $item =are=> $r" );
                }
                ...
                my ( $X, $V, $Y ) = $item =~ /^(.*?) =(.*?)=> (.*)/;
                if (   ( getparam('acceptUrl') !~ /REQUIRE/ )
                    or ( $Y =~ /(http|ftp|mailto|telnet|file):/ ) )
                {
                    &set( $V, $X, $Y );
        ...
        or ( $addressed and $message =~ s/^(no,?\s+)//i ) )
    {
        ...
        &status(
"correction is plausible, initial negative and nick deleted ($1)"
    ...
    if ($addressed) {
        ...
        if ( $message =~ /^showmode/i ) {
            if ( $msgType =~ /public/ ) {
                if ( ( getparam('addressing') ne 'REQUIRE' )
                    or $addressed )
                {
                    &performSay( $who
        ...
        if ( getparam('continuity') and $who eq $lastaddressedby ) {
            ...
            if ( $diff < getparam('continuity') ) {
                ...
                &status(
"assuming continuity of address by $who ($diff seconds elapsed)"
                );
                $continuity = 1;
            }
        } else {
...
# if (($message =~ s/^\S+\s*:\s+//) or ($message =~ s/^\S+\s+--?\s+[.\d]//)) {
    ...
    $message =~ s/^heya*,*( folks)?,*\.* *//i
      ;    # clear initial filled pauses & stuff
    $message =~ s/^[uh]+m*[,\.]* +//i;
...
# the thing to tell someone about ($tell_obj).  Yes i know these are evil globals. --kl
    ...
    if ( getparam('allowTelling') ) {
        ...
        if ( $message =~ /^tell\s+(\S+)\s+about\s+(.*)/i ) {
            ...
            /tell\s+(\S+)\s+where\s+(\S+)\s+can\s+(\S+)\s+(.*)/i )
        {

            # i'm sure this could all be nicely collapsed
            ( $target, $tell_obj ) = ( $1, $4 );
        } elsif ( $message =~
            /tell\s+(\S+)\s+(what|where)\s+(.*?)\s+(is|are)[.?!]*$/i )
        {
    ...
    if ( not defined $result ) {
        ...
        if (   $continuity
            or $addressed
            or ( getparam('addressing') ne "REQUIRE" ) )
        {
   ...
   # instead of reporting karma.  Assigned to boojum at the moment.
           ...
           # example) being ignored.   -- Masque, 12Apr2K
            ...
            if (
                ...
                and !$tell_obj
                and !$karma
                and ( $input_message_length <
                    getparam('minVolunteerLength') )
              )
            {
                $in = '';
                return 'NOREPLY';
        ...
        if ($caughtBy) {
            if ($tell_obj) {
                $message = $tell_obj;
                &status(
                    "$caughtBy: <$who>->$target<  [$message] -> $result"
                );
            } else {
    ...
    if ( defined $result ) {
        ...
        return "No authorization to teach"
          unless ( IsFlag("t") eq "t" );
        ...
        if ( ( $who eq 'NOREPLY' ) || ( $result eq 'NOREPLY' ) ) {
            return '';
        }

        return 'NOREPLY'
          if grep $_ eq $who, split /\s+/, $param{friendlyBots};
    ...
    if ( $addressed and not $caughtBy ) {
        ...
        if ( $msgType =~ /public/ ) {
            &say( "$who: " . $confused[ int( rand(@confused) ) ] )
              if $addressed;
        } else {
