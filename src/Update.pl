                ...
                if ( length($rhs) > getparam('maxDataSize') ) {
                    if ( $msgType =~ /public/ ) {
                        if ($addressed) {
                            if ( rand() > 0.5 ) {
                                &performSay(
                                    "that is too long, " . $who );
                            } else {
                                &performSay(
"i'm sorry, but that's too long, $who"
                                );
                            }
                        }
                    } else {
                ...
                $updateCount++;
                &status(
"update: <$who> \'$lhs =is=> $rhs\'; was \'$exists\'"
                );
                &set( "is", $lhs, $rhs );
            } else {    # not "also"
                if (   ( $correction_plausible == 0 )
                    && ( $exists ne $rhs ) )
                {
                    if ($addressed) {
                        if ( not $main::googling ) {
                            ...
                            &status(
"FAILED update: <$who> \'$lhs =$verb=> $rhs\'"
                            );
                        }
                    } else {
                        &status(
"FAILED update: <$who> \'$lhs =$verb=> $rhs\' (not addressed, no reply)"
                        );
...
"update: <$who> '$lhs =is=> $rhs\'; was \'$exists\'"
            ...
            if ( $factoidCount == 31337 ) {    # particular count
                ...
                &say(
"That would be factoid $factoidCount given on $mySaveChannel by $who."
                );
                &status(
"FACTOID NUMBER $factoidCount on channel $mySaveChannel by $who."
                );
                &say("woohoo!");
        ...
        if ( $verb eq "are" ) {
            ...
            if ( $exists = &get( "are", $lhs ) ) {
                if ($also) {
                    ...
                    if ( $msgType =~ /public/ ) {
                        &performSay("okay, $who.")
                          unless $rhs eq $exists;
                    } else {
                        &msg( $who, "okay." );
                    }
                    $updateCount++;
                    &status(
"update: <$who> \'$lhs =are=> $rhs\'; was \'$exists\'"
                    );
                    &set( "are", $lhs, $rhs );
                } else {    # not 'also'
                    if (   ( $correction_plausible == 0 )
                        && ( $exists ne $rhs ) )
                    {
                        if ($addressed) {
                            ...
                            &status(
"FAILED update: $lhs $verb $rhs (not addressed, no reply)"
                            );
